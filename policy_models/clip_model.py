...
from .network_modules import (CLIPImageEncoder, 
                             PositionalEncoding)
...
class CLIPTransformer(nn.Module):
    def __init__(self, token_dim=128, state_dim=8, action_dim=7, beacon_dim=3, num_heads=4, num_layers=2, sequence_length=5):
        ...
        self.image_encoder = CLIPImageEncoder(output_dim=token_dim)
    ...
    def feature_encoder(self, data):        
        ...
        phi = self.image_encoder(data['image_rgb']).unsqueeze(dim=-2)


        action_tokens = self.action_token.repeat(batch, 1, 1)

        tokens = torch.cat((states, phi, action_tokens), dim=-2)
