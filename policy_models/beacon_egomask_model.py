...
class BeaconEgoMaskTransformer(BaseTransformerPolicy):
    def __init__(self, token_dim=128, 
                 ...
                 sequence_length=5, 
                 use_ee=False):
        super(BeaconEgoMaskTransformer, self).__init__()
        ...
        self.name = 'beacon_egomask_transformer'
    ...
    def feature_encoder(self, data):        
        ...
        image_mask = torch.repeat_interleave(data['image_ego_mask'], 3, dim = 1)
        x_segmen = data['image_ego'] * image_mask

        ego_phi = self.ego_encoder(x_segmen).unsqueeze(dim=-2)
    ...
    def calculate_loss(self, data, play_data=None):
        ...
        b_hat, action = self.forward(data['obs'])
    ...
    def get_action(self, buffer):
        ...
        with torch.no_grad():
            b_hat, action = self.forward(buffer)
